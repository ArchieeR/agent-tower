...
- Agent/MCP clients receive an opaque random 256-bit capability token. The service stores only its hash, session ID, canonical member/runtime/resource/action scope, issue/expiry and revocation state. No signing or verification secret enters an agent environment or CLI output. Runtime scope uses `{adapterId, hostId, hostRuntimeId, runtimeSessionId?, hostIdentityId?}` with opaque host-owned runtime IDs; it is not limited to a universal Buzz/Hermes enum. A Buzz-hosted Hermes runtime and an independently hosted Hermes runtime are distinct bindings.
...
- `change.prepare` requires an explicit `organization.change.propose` grant plus change-kind and subject/resource scope. Owner/operator CLI may prepare through its authenticated local principal. Before owner-principal bootstrap exists, a direct trusted local CLI requester is recorded honestly as `local-operator`, never `owner`; locality does not prove approval. Proposal authority never implies approval/apply authority, and the requester cannot satisfy an independent approval requirement.
...
The TypeScript shared module is the Agent Tower reference implementation, not an instruction for Rust/Buzz to duplicate policy or approval hashing. Control Core computes and semantically validates Agent Tower change/approval/plan digests. A native host receives bounded opaque IDs/digests plus the authorized native operation and validates syntax, expiry, correlation, host idempotency/CAS and readback. Cross-language digest recomputation is forbidden unless a separate language-neutral canonicalization specification, domain registry and identical TypeScript/Rust golden vectors are accepted. Host-native source revisions/digests remain host evidence.
...
Support is operation-specific. Missing/unknown operation evidence fails closed. Canonical v1 operation records emit every field; the strict parser rejects missing fields and never defaults `requiresOwnerReview` or any guarantee. A separately versioned source-compatibility translator may map a known legacy shape to explicit `unknown` values before strict parsing, but must preserve provenance/warnings and cannot make the result stronger. `support: supported` proves only that a native invocation surface exists; it does not prove principal authorization, host authentication, runtime readiness or Agent Tower approval. Control Core owns typed operation requirements and admits a plan only when the exact mapped operation exists and every observed guarantee satisfies the fieldwise requirement. Unknown, missing or weaker values fail closed. Agent Tower may always require stronger owner review than the host.
