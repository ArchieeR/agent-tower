...
- Implemented `buzz-org` compatibility seam: the standalone package projects normalized source-owned agents, teams, channels and freshness into a versioned payload. Agent Tower strictly validates the complete payload and joins it with separately stored owner-reviewed assignments/grants through `assembleBuzzOrgCompatibilityPayload`. Channel changes participate in snapshot revisioning, and the department UI resolves teams/channels by canonical IDs rather than names. The accepted native producer boundary is a separate pure Rust adapter/export module adjacent to the unchanged UI producer (`desktop/src-tauri/src/commands/organization_export.rs`): source `buzz-desktop-tauri`, 5000 ms freshness window, raw source team/channel IDs, connected health when rejection count is zero and degraded with safe rejection-count detail otherwise. It maps only the already-safe Organization DTO and never rereads stores. Since `ManagedAgentRecord` has no separate persistent non-secret instance ID and `backend_agent_id` is optional/provider-specific, exported `managedAgentId` is exactly `buzz-agent:<lowercase pubkey>`. The safe `runtime` value comes from `record.runtime` and is a bounded runtime ID, never `agent_command_override`, a command, or an executable path; Agent Tower also validates this format fail-closed. Buzz emits only its own runtime identity; Agent Tower/Hermes launch binding joins Hermes identity separately.
...
- Identified transport gap (2026-08-20): `get_organization_export` is currently a webview-internal Tauri command, not an externally supported handoff. Accepted v0.1 proposal, pending owner approval, is a native Organization action `Export safe organization snapshot…` that maps the existing safe facts through the one pure exporter and writes the exact allowlisted JSON envelope through an owner-selected OS save dialog. This is an explicit one-time export only—no clipboard default, upload, raw-store read, unauthenticated HTTP endpoint or continuous synchronization. The durable follow-up is atomic safe snapshot publication or authenticated/session-bound IPC with permissions and freshness. A source change, full CI and separately approved Preview rebuild/install are required before use.
