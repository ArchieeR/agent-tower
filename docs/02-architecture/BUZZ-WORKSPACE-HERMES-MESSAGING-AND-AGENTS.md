...
- Native Buzz PR #1 is merged at `21af77f0423ada62abd64c1225aa35049636d993` and source-only prerelease `agent-tower-preview-v0.1.0` is published. Its Organization route is still fixture-backed/read-only; the next fork-side step is to call the shared core contract through Tauri rather than copy the fixture into a second live model.
- Native Buzz should use its safe Tauri projections (`list_managed_agents`, optionally `list_teams`) rather than reading managed-agent/team persistence files in React. The existing Agent Tower file adapter remains an external compatibility fallback only.
