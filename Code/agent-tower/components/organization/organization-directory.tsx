...
import { useOrganizationSelection } from "@/lib/selection-store"
...
function TeamNode({ department, selected, onSelect, className = "org-node-department" }: { department: DepartmentView; selected: boolean; onSelect: () => void; className?: string }) {
  ...
  const agentSeats = Array.from({ length: Math.max(department.capacity - 1, 0) }, (_, index) => department.desiredRoles[index + 1] ?? `Open seat ${index + 2}`)
  const assignedAgentSeatCount = department.memberIds.filter((id) => !department.managerMemberIds.includes(id)).length
  ...
  return (
    <div className={`org-node org-team-node ${className} accent-${department.accent} ${selected ? "is-selected" : ""}`}>
      <button aria-label={`${department.name}. Manager: ${department.desiredRoles[0]}. ${department.memberIds.length} of ${department.capacity} members.`} className="org-team-main" onClick={onSelect}>
        <span className={`manager-mini ${department.managerMemberIds.length ? "is-assigned" : "is-empty"}`} title={`Manager agent: ${department.desiredRoles[0]}`}><Bot size={15} /></span>
        <span className="org-team-surface">
          <span className="org-team-heading"><span className="org-node-icon"><Icon size={16} /></span><strong>{department.name}</strong><em>{department.memberIds.length}/{department.capacity}</em></span>
          <span className="mini-agent-row" aria-label="Agent seats">{agentSeats.map((role, index) => <i className={index < assignedAgentSeatCount ? "is-assigned" : "is-empty"} key={role} title={role}><Bot size={13} /></i>)}</span>
        </span>
      </button>
      <span className="mini-connector-row" aria-label="Tools and connectors">
        {capabilitySlots.map((entry, index) => entry ? <i key={entry.id} title={`${entry.name} · ${entry.state}`}><CapabilityGlyph kind={entry.kind} /></i> : <i aria-hidden="true" className="is-empty" key={`empty-tool-${index}`} />)}
        <button aria-label={`Add tools to ${department.name}`} className="add-tool-mini" onClick={onSelect} title="Add tools"><Plus size={14} /></button>
      </span>
    </div>
  )
}
...
function OrgChart({ model, selectedDepartmentId, onSelect, onOpenCouncil, onOpenRole, sync }: { model: OrganizationReadModel; selectedDepartmentId?: string; onSelect: (id: string) => void; onOpenCouncil: () => void; onOpenRole: (id: string) => void; sync: { status: OrganizationSyncStatus; lastSyncedAt: string; error?: string; refresh: () => Promise<void> } }) {
  const leadership = model.departments.find((department) => department.id === "leadership")
  const departmentTeams = model.departments.filter((department) => !["leadership", "knowledge", "data-centre"].includes(department.id))
  const systemServices = model.departments.filter((department) => ["knowledge", "data-centre"].includes(department.id))
  ...
  return (
    <section className="org-chart-section" aria-label="Organization chart">
      <div className="chart-toolbar">
        <h1 className="chart-title">Agent Tower</h1>
      ...
      <div className="org-chart-scroll" ref={viewportRef}>
        <div className="org-chart-canvas" ref={canvasRef} style={{ transform: `translateX(-50%) scale(${chartScale})` }}>
          ...
          <div className="chart-top-tier">
            {leadership && <TeamNode department={leadership} selected={selectedDepartmentId === leadership.id} onSelect={() => onSelect(leadership.id)} className="org-node-leadership" />}
...
function DepartmentDetail({ department, model }: { department: DepartmentView; model: OrganizationReadModel }) {
  const members = department.memberIds.map((id) => model.members.find((member) => member.id === id)).filter((member): member is OrganizationMemberView => Boolean(member))
  ...
  const toolCapabilities = capabilities.filter((entry) => ["tool", "software", "runtime", "connector", "report"].includes(entry.kind))
  const knowledgeCapabilities = capabilities.filter((entry) => entry.kind === "knowledge")
  const skillIds = Array.from(new Set([...department.skillIds, ...members.flatMap((member) => member.skillIds)]))
  const routineIds = Array.from(new Set([...department.routineIds, ...members.flatMap((member) => member.routineIds)]))
  const skillCount = skillIds.length
  const routineCount = routineIds.length
  const toolGrantCount = new Set(members.flatMap((member) => member.toolGrantIds)).size
  const seats = Array.from({ length: department.capacity }, (_, index) => ({ member: members[index], role: department.desiredRoles[index] ?? `Open seat ${index + 1}` }))

  return (
    <div className={`department-bento has-configuration accent-${department.accent}`}>
      <section className="bento-card bento-members">
        <header className="bento-header"><span className="bento-icon"><Bot size={19} /></span><div><h3>Team · {members.length}/{department.capacity}</h3></div></header>
        <div className="member-seat-row">
          {seats.map(({ member, role }, index) => {
            const profile = profiles.find((item) => item.title === role)
            const initials = role.startsWith("Open seat") ? "+" : role.split(/\s+/).map((word) => word[0]).join("").slice(0, 2)
            const seatState = member ? "is-assigned" : profile ? "is-role" : "is-open"
            return <article className={`member-seat ${seatState}`} key={`${role}:${index}`}>
              {member ? <MemberAvatar member={member} accent={department.accent} /> : <span className={`agent-avatar role-avatar accent-${department.accent}`}>{initials || "+"}</span>}
              <strong>{member?.name ?? role}</strong>
              {profile?.model && <small>{profile.model}</small>}
            </article>
          })}
        </div>
      </section>
      ...
      <section className="bento-card bento-skills">
        <header className="bento-header"><span className="bento-icon"><Sparkles size={18} /></span><div><small>Capabilities</small><h3>Skills</h3></div><strong>{skillCount}</strong></header>
        <div className="compact-square-stack" aria-label="Assigned skills">{skillIds.length ? skillIds.slice(0, 5).map((id) => <span key={id} title={id}><Sparkles size={17} /></span>) : <span className="is-empty" title="No skills assigned"><Sparkles size={17} /></span>}</div>
      </section>

      <section className="bento-card bento-routines">
        <header className="bento-header"><span className="bento-icon"><CalendarClock size={18} /></span><div><small>Cadence</small><h3>Routines</h3></div><strong>{routineCount}</strong></header>
        <div className="compact-square-stack" aria-label="Assigned routines">{routineIds.length ? routineIds.slice(0, 5).map((id) => <span key={id} title={id}><CalendarClock size={17} /></span>) : <span className="is-empty" title="No routines assigned"><CalendarClock size={17} /></span>}</div>
      </section>

      <section className="bento-card bento-tools">
        <header className="bento-header"><span className="bento-icon"><Wrench size={18} /></span><div><small>Provisioning</small><h3>Tools, software & reports</h3></div><strong>{toolCapabilities.length}</strong></header>
        <div className="bento-chip-list">{toolCapabilities.map((entry) => <span className={entry.kind === "report" ? "state-report" : `state-${entry.state}`} key={entry.id} title={`${entry.kind} · ${entry.provider} · ${entry.evidence}`}><b><CapabilityGlyph kind={entry.kind} /></b><strong>{entry.name}</strong><i>{entry.kind === "report" ? "report only" : entry.state}</i></span>)}</div><span className="bento-foot">{toolGrantCount} direct member grants · assignment ≠ health</span>
      </section>


      <section className="bento-card bento-knowledge">
        <header className="bento-header"><span className="bento-icon"><BookOpen size={18} /></span><div><small>Sources</small><h3>Knowledge</h3></div><strong>{knowledgeCapabilities.length}</strong></header>
        {knowledgeCapabilities.length ? knowledgeCapabilities.map((entry) => <div className="knowledge-row" key={entry.id}><span>{entry.name}</span><i className={`state-${entry.state}`}>{entry.state}</i></div>) : <p>No knowledge sources assigned.</p>}
        <span className="bento-foot">Permission-scoped search · read · cite</span>
      </section>

      <section className="bento-card bento-roles">
        <header className="bento-header"><span className="bento-icon"><Network size={18} /></span><div><small>Operating roster</small><h3>Roles & models</h3></div></header>
        <div className="role-model-table"><span>Role</span><span>Member</span><span>Primary model</span>{department.desiredRoles.map((title) => { const profile = profiles.find((role) => role.title === title); const member = members.find((item) => item.role === title); return <div className="role-model-row" key={title}><strong>{title}</strong><span>{member?.name ?? "Unassigned"}</span><code>{profile?.model ?? "Pending"}</code></div> })}</div>
      </section>

      <section className="bento-card bento-buzz">
        <header className="bento-header"><span className="bento-icon"><PlugZap size={18} /></span><div><small>Integration</small><h3>Buzz</h3></div><i className={buzzTeam ? "state-healthy" : "state-planned"}>{buzzTeam ? "Mapped" : "Pending"}</i></header>
        <dl><div><dt>Target</dt><dd>{buzzTeam?.name ?? "Not mapped"}</dd></div><div><dt>Members</dt><dd>{buzzTeam?.memberIds.length ?? 0}</dd></div><div><dt>Review</dt><dd>Owner required</dd></div></dl>
        <a className="primary-action" href="#department-buzz-draft"><ShieldCheck size={15} />Prepare owner review</a>
      </section>
      ...
      <section className="bento-card bento-activity">
        <header className="bento-header"><span className="bento-icon"><Activity size={18} /></span><div><h3>Activity</h3></div></header>
        <table className="activity-table"><thead><tr><th>Time</th><th>Activity</th><th>Source</th><th>State</th></tr></thead><tbody><tr><td colSpan={4}>No activity yet</td></tr></tbody></table>
      </section>
    </div>
  )
}
...
export function OrganizationDirectory({ model }: { model: OrganizationReadModel }) {
  ...
  const activeDepartment = useMemo(() => activeDetail?.kind === "department" ? currentModel.departments.find((department) => department.id === activeDetail.id) : undefined, [activeDetail, currentModel.departments])
  ...
  return (
    <div className="organization-page">
      ...
      {activeDepartment && <DetailModal accent={activeDepartment.accent} title={activeDepartment.name} onClose={closeDetail}><DepartmentDetail department={activeDepartment} model={currentModel} /></DetailModal>}
